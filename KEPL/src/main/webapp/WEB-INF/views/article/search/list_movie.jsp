    ...
    <div class="container content">
	    <div class="content-header">
	      <div class="container-fluid">
	        <div class="row mb-2">
	          <div class="col-sm-6">
	            <h1 class=categoryfont>영화 게시판</h1>
	          </div><!-- /.col -->
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-header">
								<form action="${path}/article/search/list/movie" method="get">
									...
									<ul class="pagination justify-content-center m-0">
										<c:if test="${pageMaker.prev}">
											<li class="page-item"><a class="page-link"
												href="${path}/article/search/list/movie${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
										</c:if>
										<c:forEach begin="${pageMaker.startPage}"
											end="${pageMaker.endPage}" var="idx">
											<li class="page-item"
												<c:out value="${pageMaker.criteria.page == idx ? 'class=active' : ''}"/>>
												<a class="page-link" href="${path}/article/search/list/movie${pageMaker.makeSearch(idx)}">${idx}</a>
											</li>
										</c:forEach>
										<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
											<li class="page-item"><a class="page-link"
												href="${path}/article/search/list/movie${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
										</c:if>
									</ul>
								</nav>
							</div>
...
function selectAsc($howAsc) {
	location.replace("${path}/article/search/list/movie?howAsc=" + $howAsc);
}
...
$(document).ready(function () {
    ...
    $("#searchBtn").on("click", function (event) {
        self.location =
            "${path}/article/search/list/movie${pageMaker.makeQuery(1)}"
